...
:guilabel:`Customers` page, or by clicking on the customer's name on a sales order reveals that
...
Next, navigate to the :menuselection:`Sales app --> Products --> Products` and select the product
for which multiple prices should be applied. Selecting a product from the :guilabel:`Products` page
...
.. note::
   ...
   it is also reflected on the pricelist itself.
...
.. example::
   ...
   the :guilabel:`Discount` to `-100` and the :guilabel:`Margins` to `5`. This is often seen in
   retail situations.
...
.. example::
   To apply 20% discounts, with prices rounded up to 9.99, set the :guilabel:`Discount` field to
   `20`, the :guilabel:`Extra Fee` field to `-0.01`, and the :guilabel:`Rounding Method` field to
   `10`.
