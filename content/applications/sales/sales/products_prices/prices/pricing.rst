...
CoquiAPPs has a powerful pricelist feature to support a pricing strategy tailored to your business.
A pricelist is a list of prices or price rules that CoquiAPPs searches to determine the suggested price.
...
.. note::
   * Once again the system is smart. If a rule is set for a particular item and another one for its
     category, CoquiAPPs will take the rule of the item.
...
Pricelists can be set by countries group. Any new customer recorded in CoquiAPPs gets a default
pricelist, i.e. the first one in the list matching the country. In case no country is set for the
customer, CoquiAPPs takes the first pricelist without any country group.
