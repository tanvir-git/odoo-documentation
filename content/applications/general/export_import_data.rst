...
Export data from CoquiAPPs
=====================

When working with a database, it sometimes is necessary to export your data in a distinct file.
Doing so can help to do reporting over your activities (even if CoquiAPPs offers a precise and easy
reporting tool with each available application).

With CoquiAPPs, you can export the values from any field in any record. To do so,
...
.. image:: export_import_data/export-data-overview.png
   :align: center
   :alt: overview of all the options to take into account when exporting data in CoquiAPPs
...
Import data into CoquiAPPs
=====================

How to start
------------

You can import data on any CoquiAPPs's business object using either Excel
...
* When you add a new column, CoquiAPPs might not be able to map it automatically if its
  label doesn't fit any field in CoquiAPPs. Don't worry! You can map
...
and map it to the **ID** (External ID) column in CoquiAPPs.
...
CoquiAPPs tries to find with some heuristic, based on the first ten lines of
...
CoquiAPPs can automatically detect if a column is a date, and it will try to guess the date format from a
...
To view which date format CoquiAPPs has found from your file you can check the **Date Format** that is
...
.. note::
   ...
   the date format is correct in CoquiAPPs whatever your locale date format is.
...
currency sign attached to them. CoquiAPPs also automatically detect which thousand/decimal separator you
use (you can change those under **options**). If you use a currency symbol that is not known to
CoquiAPPs, it might not be recognized as a number though and it will crash.
...
Note that if your CSV file has a tabulation as separator, CoquiAPPs will not detect the separations. You
...
link to a record of the 'Country' object. When you want to import such fields, CoquiAPPs will have to
recreate links between the different records. To help you import such fields, CoquiAPPs provides three
mechanisms. You must use one and only one mechanism per field you want to import.

For example, to reference the country of a contact, CoquiAPPs proposes you 3 different fields to import:

- Country: the name or code of the country
- Country/Database ID: the unique CoquiAPPs ID for a record, defined by the ID postgresql column
...
imports. CoquiAPPs will take care of creating or modifying each record depending if it's new or not.

This feature allows you to use the Import/Export tool of CoquiAPPs to modify a batch of records in your
favorite spreadsheet application.

What happens if I do not provide a value for a specific field?
--------------------------------------------------------------

If you do not set all fields in your CSV file, CoquiAPPs will assign the default value for every non
defined fields. But if you set fields with empty values in your CSV file, CoquiAPPs will set the EMPTY
value in the field, instead of assigning the default value.

How to export/import different tables from an SQL application to CoquiAPPs?
----------------------------------------------------------------------
...
To manage relations between tables, you can use the "External ID" facilities of CoquiAPPs. The "External
...
The two files produced are ready to be imported in CoquiAPPs without any modifications. After having
...
Feel free to use the one of your previous software to ease the transition to CoquiAPPs.
