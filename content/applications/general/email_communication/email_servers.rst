====================================================
Send and receive emails in CoquiAPPs with an email server
====================================================

CoquiAPPs Online or CoquiAPPs.sh users
============================

Since **CoquiAPPs sets up its own mail servers for the database**, outgoing and incoming emails already
work out-of-the-box. So for **CoquiAPPs Online** and **CoquiAPPs.sh** customers, nothing needs to be
configured!

Unless an external mail server is required to send large batches of mass emails, simply use the
standard online CoquiAPPs database normally since it has already been pre-configured to send email.

.. important::
   The CoquiAPPs server is subject to a daily email limit to prevent abuse. The default limit is 200
...
This document is **mainly dedicated to CoquiAPPs on-premise databases** who don't benefit from an
out-of-the-box solution to send and receive emails in CoquiAPPs, unlike `CoquiAPPs Online <https://www.CoquiAPPs.
com/trial>`_ and `CoquiAPPs.sh <https://www.CoquiAPPs.sh>`_. Incoming and outgoing servers must be configured
...
.. warning::
   If no one in the company is employed to manage email servers, CoquiAPPs Online and CoquiAPPs.sh are
   strongly recommended. In these CoquiAPPs hosting types email sending and receiving works instantly
   ...
   :doc:`Configure DNS records to send emails in CoquiAPPs <email_domain>`
...
Documents in CoquiAPPs (such as a CRM opportunity, a sales order, an invoice, etc.) have a discussion
...
the reply updates the chatter, and CoquiAPPs relays another reply to the followers as a notification.
...
As a system admin, go to :menuselection:`Settings --> General Settings --> Discuss` in CoquiAPPs, and
...
record in CoquiAPPs. Reference the SMTP data of the external email server. Once all the information has
...
Note that port 25 is blocked for security reasons on the CoquiAPPs Online and CoquiAPPs.sh platforms. Try
...
message, CoquiAPPs will try to redistribute that same email to the other subscribers in the thread.
However, if the domain `customer.example.com` forbids that kind of usage for security, the email
that CoquiAPPs is trying to redistribute would get rejected by some recipients' email servers.

To avoid that problem, CoquiAPPs sends all emails using a "From" address from the same authorized
domain.
...
.. note::
   The `mail.default.from_filter` works only for `CoquiAPPs-bin` configurations or the default CoquiAPPs email
...
.. example::
   ...
   notifications configuration in CoquiAPPs: ``“Admin” <admin@example.com>`` => ``“Admin”
...
depending on the :guilabel:`From` email address or domain that CoquiAPPs is sending on behalf of. This
...
domains in a multi-company environment. Access this field in CoquiAPPs by navigating to
...
When an email is sent from CoquiAPPs while the :guilabel:`FROM Filtering` field is set, an email server
is chosen in the following sequence:

#. First, CoquiAPPs searches for an email server that has the same :guilabel:`FROM Filtering` value as
...
#. However, if no email servers are found that use the :guilabel:`From` value, then CoquiAPPs searches
...
If no email servers are found after checking for the domain, then CoquiAPPs returns all email servers
that do not have any :guilabel:`FROM Filtering` value(s) set.

Should this query return no results, then CoquiAPPs performs a search for an email server using the
system parameter: `mail.default.from`. First, the email address listed attempts to match an email
server, and then the domain attempts to find a match. If no email server is found, CoquiAPPs returns the
first outgoing email server (sorted by priority).

.. note::
   If several email servers are found, then CoquiAPPs uses the first one according to its priority. For
...
In CoquiAPPs a separate email server can be used for transactional emails and mass
...
Choose the appropriate email server. With these settings, CoquiAPPs uses the server with the lower
...
CoquiAPPs relies on generic email aliases to fetch incoming messages.

* **Reply messages** of messages sent from CoquiAPPs are routed to their original discussion thread (and
  ...
  catchall alias (`catchall@mycompany.CoquiAPPs.com`). The catchall address, however, does not have
...
* **Bounced messages** are used as a Return-Path. One example this is especially
  useful for is in `CoquiAPPs Email Marketing <https://coqui.cloud/page/email-marketing>`__. In this
...
* **Original messages**: several business objects have their own alias to create new records in CoquiAPPs
  from incoming emails:

   * Sales channel (to create *Leads* or *Opportunities* in `CoquiAPPs CRM <https://coqui.cloud/page/
     crm>`_)
   * Support channel (to create *Tickets* in `CoquiAPPs Helpdesk <https://coqui.cloud/page/helpdesk>`_)
   * Projects (to create new *Tasks* in `CoquiAPPs Project <https://coqui.cloud/page
     /project-management>`_)
   * Job positions (to create *Applicants* in `CoquiAPPs Recruitment <https://coqui.cloud/page
     /recruitment>`_)

Depending on the mail server, there might be several methods to fetch emails. The easiest and most
recommended method is to manage one email address per CoquiAPPs alias in the mail server.
...
* Set the :guilabel:`Alias Domain` name in :menuselection:`Settings --> General Settings -->
  ...
  database.
* If the database's hosting type is CoquiAPPs on-premise, create an :guilabel:`Incoming Mail Server` in
  CoquiAPPs for each alias. To create a new incoming server go to: :menuselection:`Settings --> Discuss
  ...
  .. image:: email_servers/incoming-server.png
     :align: center
     :alt: Incoming mail server configuration on CoquiAPPs.

* If the database's hosting type is CoquiAPPs Online or CoquiAPPs.sh, redirecting or forwarding incoming
  messages to CoquiAPPs's domain name instead of the external email server is recommended. That way,
  incoming messages can be received without delay. Redirections for all email addresses should be
  set to CoquiAPPs's domain name in the email server (e.g. `catchall\@mydomain.ext` to
  `catchall\@mycompany.CoquiAPPs.com`).

All the aliases are customizable in CoquiAPPs. Object aliases can be edited from their respective
