...
In CoquiAPPs, every transaction is recorded in the default currency of the company, and reports are all
based on that default currency. When you have a bank account in a foreign currency, for every
transaction, CoquiAPPs stores two values:

-  The debit/credit in the currency of the *company*;
-  The debit/credit in the currency of the *bank account*.

Currency rates are updated automatically using the web services of a banking institution. By
default, CoquiAPPs uses the European Central Bank's web services but other options are available.
...
Once CoquiAPPs is configured to support multiple currencies, they are all created by default, but not
...
Upon creation of the journal, CoquiAPPs automatically links the bank account to the journal. It can be
...
field and register the payment. CoquiAPPs automatically creates and posts the foreign **exchange gain or
...
.. note::
   Note that you can pay a foreign bill with another currency. In that case, CoquiAPPs automatically
...
CoquiAPPs's rate. To do so, simply click on :guilabel:`Reset to CoquiAPPs's Rate`.

.. image:: foreign_currency/foreign-reset-rates.png
   :align: center
   :alt: Banner to reset back to CoquiAPPs's rates.
...
You can set the date of the report in the :guilabel:`Date` field. CoquiAPPs automatically reverses the
