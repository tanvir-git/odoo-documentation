...
In CoquiAPPs Manufacturing, multiple :abbr:`BoMs (Bills of Materials)` can be linked to each product, so
...
components, CoquiAPPs gives the option to specify a product variant in the :guilabel:`Apply on Variants`
