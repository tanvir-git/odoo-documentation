...
In CoquiAPPs, there are two strategies for replenishing inventory: **Reordering Rules** and the **Make
...
reaches its minimum, CoquiAPPs automatically generates a purchase order with the quantity needed to
...
Unlike products replenished using reordering rules, CoquiAPPs automatically links the sales order to the
purchase order (PO) or manufacturing order (MO) generated by the MTO route. Another difference
between reordering rules and MTO is with MTO, CoquiAPPs generates a draft :abbr:`PO (Purchase Order)` or
:abbr:`MO (Manufacturing Order)` immediately after the sales order is confirmed. With reordering
rules, CoquiAPPs generates a draft :abbr:`PO (Purchase Order)` or :abbr:`MO (Manufacturing Order)` when
the product's forecasted stock falls below the set minimum quantity. In addition, CoquiAPPs will
...
reordered. If there is no specific rule for a product, CoquiAPPs assumes that the :guilabel:`Minimum
...
.. note::
   For products that don't have a set reordering rule, CoquiAPPs calculates the forecast as sales orders,
   deliveries, and receipts are confirmed. For products that have a set reordering rule, CoquiAPPs
...
levels tracked, so CoquiAPPs can not account for a consumable product in the stock valuation.
...
To automate replenishment, click :guilabel:`Automate Orders`. When this button is clicked, CoquiAPPs
...
To activate the Make to Order (MTO) route in CoquiAPPs:
