...
When a product movement needs to be done, CoquiAPPs finds available products that can be assigned to
the transfer. The way CoquiAPPs assigns these products depends on the *Removal Strategy* defined in
...
those with the closest date first is preferred. Depending on the chosen removal strategy, CoquiAPPs
...
CoquiAPPs requests a transfer for the products that have been in the stock for the longest period.
...
When a sales order for 25 units of Hand Cream is created, CoquiAPPs automatically reserves the lots with
